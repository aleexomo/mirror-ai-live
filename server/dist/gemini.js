...
const getLangInst = (lang) => `Emma the Stylist instruction: All generated text must be in ${lang === 'pt' ? 'Portuguese' : lang === 'es' ? 'Spanish' : lang === 'ja' ? 'Japanese' : 'English'}. Brand 'Everyday Mirror' remains in English.`;
const STYLE_PROTOCOL = 'CRITICAL: Do NOT add, modify, or emphasize any facial hair (beards, stubble, or mustaches). Maintain a clean, sophisticated, and feminine aesthetic. Focus strictly on requested changes.';
const PRESERVATION_PROTOCOL = 'CRITICAL: Maintain the exact body shape, weight, height, and pose of the person. Do NOT change the background or environment. The person must be 100% recognizable as themselves in their current setting.';
...
const getImageUrlFromResponse = (response) => {
    ...
    if (candidate.finishReason === 'SAFETY') {
        throw new Error("I couldn't generate this specific look due to safety guidelines. Please try a different photo or style!");
    }
...
export async function generateInitialGreeting(base64Image, mode, lang) {
    const ai = aiFromEnv();
...
export async function generateMakeupLook(base64Image, style, lang) {
    const ai = aiFromEnv();
    const basePrompt = style === 'Surprise Me'
...
export async function generateOutfitLook(base64Image, vibe, lang) {
    const ai = aiFromEnv();
...
export async function generateHairLook(base64Image, look, lang) {
    const ai = aiFromEnv();
...
export async function generateTutorialSteps(originalImage, targetImage, mode, lang) {
    const ai = aiFromEnv();
    const origData = extractBase64(originalImage);
    const targetData = extractBase64(targetImage);
    if (!origData || !targetData)
        throw new Error('Invalid image data for tutorial');
...
export async function generateShoppingItems(targetImage, mode, lang) {
    const ai = aiFromEnv();
    let itemCategory = 'makeup products';
    if (mode === 'CLOTHES')
        itemCategory = 'clothing and outfits';
    if (mode === 'HAIR')
        itemCategory = 'hair accessories and products';
    ...
    if (!imgData)
        throw new Error('Invalid image data for shopping');
    const response = await ai.models.generateContent({
...
export async function getProgressFeedback(targetImage, currentProgressImage, currentStep, mode, lang) {
    const ai = aiFromEnv();
    ...
    if (!targetData || !progressData)
        throw new Error('Invalid image data for feedback');
    const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: {
            parts: [
...
export async function askCoachQuestion(question, targetImage, mode, currentStep, lang) {
    const ai = aiFromEnv();
    ...
    const imgData = extractBase64(targetImage);
    if (!imgData)
        throw new Error('Invalid image data for QA');
    const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: { parts: [{ inlineData: { data: imgData, mimeType: 'image/jpeg' } }, { text: prompt }] },
    });
...
export async function tts(text, voiceName = 'Kore') {
    const ai = aiFromEnv();
    const response = await ai.models.generateContent({
